...
**Task** Run the following codeblock to reload the data, using every row to set the data types. Does it show any parsing errors when you run? Answer below **Answer It shows a Delimiter error and asks to retrieve full column specifications for the data.**
...
**Task** Run the following codeblock to glimpse the data. What data type is the "Vendor Name" field? What data type is the "Amount" field? What data type is the "Date" column? Answer below. **Answer The "Vendor Name" field is characters, and the amount field is doubles which is numbers. The date column is under characters because the number as a whole signifies something. It is not a numerical value that we could use to calculate statistics or math with, instead it tells us something about the data.**
...
**Task** Run the following codeblock to count the number of payments. Looking at the result, what jumps out at you as potentially problematic? **Answer The first thing I notice is the fact that there are categories with N/A as answers. These empty values could create problems in data analysis.**
...
**Task** Run the following code to arrange the grants & loan data so that the smallest amount is first. Describe what you see. **Answer I see that the smallest loan in the entire data set was for** \$60 by the Land Preservation Trust, Inc. This goes up to \$**24,614.00.**
...
**Task** Run the following codeblock to use the `clean_names()` function from janitor to standardize column names. How does the function change the headers? Answer below. **Answer The headers are all lowercase, but the headers are not changed in terms of words.**
...
**Task** Run the following codeblock to check for duplicate rows using get_dupes(). How many duplicates are possible? **Answer 50 duplicates are possible.**
...
**Task** Run the following codeblock to use distinct() to get rid of duplicate rows. How many rows does the new dataframe have? Answer below. **Answer The new data frame has 17740 rows.**
...
**Task** Run the following codeblock to use str_sub() to convert the ZIP codes that have nine digits to five digits, creating a new column. Look at the difference in the result - what changed? **Answer An extra column was added titled zip5 that only contains the first five values of the zip code entered.**
...
the only thing i struggled with was summarising the data. the amount is 767888790
