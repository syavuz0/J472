...
```{r echo=FALSE}
census_api_key("933a1b78ee9850566cf1168838c648fc7878c6b4", install=TRUE)
```
...
**Task** Run the following codeblock to calculate the percentage change and put the state with the largest percentage growth below. **Answer Nevada had the largest percentage change.**
...
**Task** Run the following codeblock to create a chart showing the margin of error for each county. The current headline isn't very descriptive of the data. What should it be? **Answer Range of Household Income in Maryland**
...
Is the difference between Calvert and Montgomery significant? **Answer No because the estimate covers the entirety of montgomery county as well.**

Is the difference between Howard and everyone else significant? **Answer Yes because everyone else is outside of their estimate.**
...
**Task** Let's look at the average household income for high schools that have 60% or more graduates immediately enrolling in college, and then look at those with less than 40%. Write a sentence describing the results. **Answer There are more students with a lower average enrollment percentage with household income, and less students with higher enrollment rates at a higher household income.**
...
**Task** What is another question you could ask of this data? **Answer How specifically does zip code, age, and point in educational career play a factor?**
