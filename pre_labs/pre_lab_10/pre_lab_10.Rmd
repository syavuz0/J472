...
**Task** Run the following code to load data. Describe what you see in the answer space below. What columns exist in this data? **Answer The columns that exists are object id, zipcode2, zip name, existing, zipcode1, shape length, shape area, and geometry,**
...
**Task** Run the following code. Describe the output in the space below: what kind of information does it contain? **Answer It contains the shape of each zipcode, its area of the region, and its length.**
...
**Task** Run the following code. Describe the output in the space below. **Answer The result is a map of MD, with each zipcode outlined.**
...
**Task** Run the following code. Describe the output in the space below. **Answer I'm assuming that this contains ID's for all counties in the U.S.**
...
**Task** Run the following code. Describe the output in the space below. **Answer This is the data for the counties in MD, including data like their GEOIDs, name, and other relevant information.**
...
**Task** Run the following code. Describe the output in the space below. **Answer This displays the county map for Maryland.**
...
**Task** Run the following code. Describe the output in the space below. **Answer This displays the zipcodes within each county in MD.**
...
**Task** Run the following code. Describe the output in the space below. **Answer This displays the number of foreclosures in each county in March.**
...
**Task** Run the following code. Describe the output in the space below. **Answer This combined the zip code data with the area and length of each shape, as well as the number of foreclosures in march for each zip.**
...
**Task** Run the following code. Describe the output in the space below. **Answer The lightest portion of the map is the region with the highest number of foreclosures int terms of zip codes in MD.**
...
**Task** Run the following code. Describe the output in the space below. **Answer the output displays the population for each county as well as it's geometry.**
...
**Task** Run the following code. Describe the output in the space below. **Answer This is basically extremely similar to the last example, but it formats the data in a more readable format within the code block when generated with the FIPS code.**
...
**Task** Run the following code. Describe the output in the space below. **Answer The output displays a time and date as well as the type of foreclosure for each county.**
...
**Task** Run the following code. Describe the output in the space below. **Answer The output gives results based on the specific geometry, bounding box, and counties with their populations.**
...
**Task** Run the following code. Describe the output in the space below. **Answer The geometry and population data is combined with the time and date foreclosure data for each county.**
...
**Task** Run the following code. Describe the output in the space below. **Answer The data is the same as before except there is a separate column/table at the bottom that describes the geometry of each shape, as well as its coordinates on the map.**
...
**Task** Run the following code. Describe the output in the space below. **Answer The map shows the number of foreclosure dates for each county through a coloring scale.**
...
**Task** Run the following code. Describe the output in the space below. **Answer The color difference in this map makes it a lot more easier to pick apart which county has the highest number of foreclosures. The x and y labels are much cleaner.**
...
**Task** Run the following code. Describe the output in the space below. What regional patterns do you see? **Answer It is difficult to make conclusions, but it seems that mostly southern Maryland has the highest foreclosure rates.**
