...
**Answer** Copy the url of the graphic you published here. It will begin with <https://datawrapper.dwcdn.net/>. Don't copy the browser's current url. <https://datawrapper.dwcdn.net/fw867/1/>
...
md_gov_county_new <- md_gov_county |>
  group_by(fips_code, county, moore) |>
  summarize(total_votes = cox + moore + lashar + wallace + harding) |>
  mutate(pct_moore_votes = moore/total_votes*100) 
...
**Task** Look for "Hide regions without data" under Appearance, and click the slider icon to enable that feature. You should see a map zoomed into Maryland with some counties in various colors. . Describe the changes from the previous map **Answer** **The changes from the previous map is that it shows descriptions and percentages through colors and a visual index of location through the counties of Maryland**
...
**Task** Click the "Customize tooltips" button so it expands down. Change {{ fips_code }} to {{ county }} and {{ pctmoore }} to {{ FORMAT(pctmoore, "0.0%")}} **Answer** Describe the changes from the previous map **When hovering over regions in the map it states the county name as well as the percentage of votes for moore.**
...
<https://datawrapper.dwcdn.net/JTW4R/1/>
