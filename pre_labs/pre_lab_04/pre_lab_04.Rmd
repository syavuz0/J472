...
**Task** Run the following code to group and count records in the Grantee column, and filter to examine only recipients that start with W. Name a problem that you see that is preventing proper grouping. **Answer I'm assuming the problem is that it's sorting based on all caps alphabetical then lowercase alphabetical, basically splitting the filter in two.**
...
**Task** Run the following code to make a copy of the Grantee field called grantee_clean, then use the `key_collision_merge` function from refinr to attempt to standardize similar values. Store it as a new object called cleaned_md_grant_loans Click the black arrow on the table to examine the Grantee and grantee_clean columns. Looking at the results on the first 3 pages only (first 30 rows), name two changes the refinr package made, by comparing Grantee to grantee_clean. **Answer There is no distinction between caps and all caps values, as well as making the names shorter and more readable based on the original data set.**
...
**Task** Run the following code to group by grantee_clean and Grantee, then filter for recipients that start with W. Name three changes the refinr function made in the first 30 results. **Answer It put the values that start with only the letters without anything after, then sorted alphabetically after. I'm not really sure what else it does.**
...
**Task** Run the following code to use the n_gram_merge() function to clean the data, then group by grantee_clean and Grantee to examine recipients that start with W. Look at the differences. Does the changes it made vary substantially from the previous use of key_collision_merge? **Answer The answers don't necessarily vary from the previous use, or I can't tell.**
...
clean_md_grant_loans \<- read_csv("data/clean_grants.csv")
