...
**Answer: It would be easier to keep track of, especially if you need to know what column is holding what kind of data.**
...
**Answer: Or provides the combined records of both department and seats. The number observed 41011 is a sum of both.**
...
A general tip about using filter: it's easier to work your way towards the filter syntax you need in steps rather than try and write it once and trust the result. Each time you modify your filter, check the results to see if they make sense. This adds a little time to your process but you'll thank yourself for doing it because it helps avoid mistakes. Note that whatever you're trying to match using `filter` - whether an exact match or a partial one - R is case-sensitive by default. Go back and try the previous block, changing the pattern to "shakespeare" to see this. **No values are shown.**
...
**Answer: Expenditure date is listed as n/a**
...
**Answer: The values in the month column are ordered Y/M/D instead one big block of numbers.**
...
**Answer Its adding the total votes for cox , moore, lashar, wallace, and the other write_ins as with ell as calculating the value for the proportion of moores votes to the total votes**
...
**Answer Its giving the percent number instead of the proportion, as well as arranging the percent of moore votes in descending order.**
...
**Answer I am unable to see the chart from the last section of code, but I would have prepared to see the number of votes in order instead, which is easier for readability**
...
**Answer str_to_lower lowercases the strings or characters. str_to_Title() uppercases the first letter of each word.**
...
**Answer it is detected different states and associating them with state abbreviations.**
...
**Answer it makes sense to me because I know where the data is coming from and what the abbreviations mean, as well as the descriptions of the columns. I do not understand why the totals are decimal numbers however.**
...
1.  How would you utilize functions to mask dirty data?

2.  Do the dates have to be in any specific format for the code to be able to read it and reformat?

3.  How complex can filtering get in terms of arranging numbers and categories?
