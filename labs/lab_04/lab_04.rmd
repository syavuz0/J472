...
media_expenses <- cleaned_md_expenses |>
  ...
  filter(str_detect(Expense.Category, 'Media')) |>
...
write.csv(media_expenses, "media_expenses_clean.csv")
...
media_expenses |>
  filter(str_detect(Payee.Name, "Facebook") |>
  group_by(amount = sum(amount)) |>
  arrange(desc(amount))
...
**A2. I'm having a bit of trouble with visualizing the data, but from scrolling through the data set, it looks like consulting fees get a really high amount of money from Harris Public Affairs. The ads are general political or have to do with government affiliation.**
...
**A3. I am not able to visualize the data because I keep getting errors about incomplete expressions, but I assume my approach to the code is correct. I apologize for the inconvenience.**

```{r}
media_expenses |>
  filter(str_detect(Expense.Toward, "Moore") |>
  group_by(amount = sum(amount)) |>
  arrange(desc(amount))
...
**A4. Unfortunately, I don't know why R cannot detect my str_detect() function, and I don't know how to solve the problem. Apologies again.**

```{r}
advertising_expenses <- md_expenses |>
  filter(str_detect(Expense.Purpose, "Online Advertising") & str_detect(Payee.Name, "Facebook")) |>
  group_by(amount = sum(amount)) |>
  summarise(amount) |>
  arrange(desc(amount))
...
**A5 One of the best stories out of this data set is the comparison between high level of money that is being spent on advertising for democratic candidates.**
