...
**A1. <https://datawrapper.dwcdn.net/k8r2B/1/>**

**My reasoning behind choosing this one is because the data includes the
percentages of each persons votes in their respective region. The
comparison paints a good picture which is why I chose the grouped bar
option.**
...
md_county_alldata <-left_join(md_county_18, md_county, by=('zipcode' = 'GEOID')) 

md_all_data_updated <- md_county_alldata |>
  mutate(total_votes= Hogan + Jealous + Quinn + Schlakman, Cox, Moore, Lashar, Wallace, Harding, Sefcik, pct_cox = Cox/total_votes*100, pct_hogan = Hogan/total_votes*100) |>
  group_by(County.x, pct_cox, pct_hogan) |>
  summarize(pct_cox)

md_all_data_updated

write_csv(md_all_data_updated, "md_all_data_updated.csv")
```
...
**A2. <https://datawrapper.dwcdn.net/FdEGg/1/>**

**I believe the story is about the increase of mail-in voting around the
year of 2020, which could possibly signify covid-19's impact on the
different styles of voting.**

```{r}

votes_type <- read_csv('data/md_votes_by_type.csv') |> clean_names()

votes_type_pct <- votes_type |>
  mutate(pct_early = early/total*100, pct_day = election_day/total*100, pct_mail = mail/total*100) |>
  group_by(year, pct_early, pct_day, pct_mail) |>
  summarise(year)

votes_type_pct

write_csv(votes_type_pct, "votes_type_pct.csv")
```
...
**A3. <https://datawrapper.dwcdn.net/r2kUX/1/> The story is that one
specific zip code has the highest number of overdose calls. A journalist
could investigate the reasoning behind the central areas of montgomery
county attracting more drug usage, as well as demographic groups most
affected by drug usage and its relevance to the area and region.**

```{r}

moco_overdoses <- read_csv('data/montgomery_2022_overdoses.csv')

moco_overdoses_geo <- moco_overdoses |> 
  group_by(zip) |>
  summarize(total_calls_zip = n())|>
  mutate(pct_calls_perzip = total_calls_zip/659*100)

moco_overdoses_geo

write_csv(moco_overdoses_geo, "moco_overdoses_geo.csv")
