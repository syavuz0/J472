...
**Q2** Which Maryland counties have a median age of at least 50? You can use the dataframe you just made to answer this, but you must write code to display only those counties with a median age of at least 50. **A2 I'm not sure why this code isn't specifically working, but my intention was to create a filter that only displays values that are above 50 for the median age column of the dataset. Looking at the previous one without the filter, I can tel that Talbot County has a median age of 51, which is the only value above 50.**
...
```{r}
moco_911_data <- read_csv("data/montgomery_2022_overdoses.csv")
...
```{pop_18_plus_acs <- pop_18_plus_acs |> left_join(zip_calls by=c('zip'='GEOID'))}
pop_18_plus_acs <- pop_18_plus_acs |> mutate(rate=(total_calls/estimate*1000) |> drop_na()
