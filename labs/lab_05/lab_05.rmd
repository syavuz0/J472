...
```{r}
baltco_911_overdose_calls <- read_csv('tabula-Baltimore County; Carey, Samantha log OD-2.csv', col_names = FALSE)
...
A1. Because all entries have at least one overdose call, the dates without any should be excluded from the dataset within my understanding.
...
A2. If days 1 and 7 are saturday/sunday (the weekend) the general trend would be that there is a higher percentage of overdose calls on the weekend.

```{r}
baltco_911_overdose_calls <- baltco_911_overdose_calls |> mutate(day_of_week = wday(date))
...
A3. Silver Spring has the most amount of overdose calls. I am unsure of the population of each region, but there are factors like crime rate, drug abuse, and population that contribute to the counts of calls.
...
A4. The best story that I could extract from this data set would be researching why the weekends correlate to higher overdose calls.
